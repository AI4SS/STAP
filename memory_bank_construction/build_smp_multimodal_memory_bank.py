"""
构建包含 visual + text + user 多模态特征的记忆库
使用PCA降维将1801维特征压缩到768维
"""
import os
import pickle
import numpy as np
...
def load_smpd_video_data():
    data_path = os.path.join(PROJECT_ROOT, 'data/processed_features')
...
def extract_multimodal_features(train_data):
    ...
    for i in tqdm(range(len(labels)), desc="提取多模态特征"):
        ...
        user = np.array(user_features[i])  # (9,)
        
        combined = np.concatenate([visual, text, user])  # (1801,)
...
def main():
    print("=" * 60)
    print("构建多模态记忆库 (visual + text + user)")
    print("=" * 60)
    
    # 加载数据
    print("\n[1] 加载 smpd-video 训练数据...")
    train_data = load_smpd_video_data()
    print(f"  训练集大小: {len(train_data['labels'])}")
    
    # 提取多模态特征
    print("\n[2] 提取多模态特征...")
    raw_features, labels = extract_multimodal_features(train_data)
    print(f"  原始特征形状: {raw_features.shape}")
    print(f"  特征组成: visual(768) + text(1024) + user(9) = 1801")
    ...
    # 保存
    output_path = os.path.join(PROJECT_ROOT, '.fixed_assets/master_grid_M6_N32_multimodal.pkl')
