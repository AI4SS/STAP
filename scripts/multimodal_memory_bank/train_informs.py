"""
GSAR-Jamba-TOPO Informs 训练脚本 - 多模态记忆库版本
...
log_dir = PROJECT_ROOT / "logs" / "multimodal_memory_bank_v2"
log_dir.mkdir(parents=True, exist_ok=True)
log_file = log_dir / f"train_informs_multimodal_{timestamp}.log"
...
logger = logging.getLogger('GSAR_Jamba_TOPO_Informs_Multimodal')
...
def compute_composite_popularity(data: Dict) -> tuple:
    ...
    for target in TARGET_COLUMNS:
        ...
        min_max[target] = {
            'min': all_labels[target].min(),
            'max': all_labels[target].max()
        }
        logger.info(f"{target}: min={min_max[target]['min']:.4f}, max={min_max[target]['max']:.4f}")
    ...
    for split in ['train', 'valid', 'test']:
        ...
        popularity = (normalized['video_play_count'] + 
                      normalized['video_heart_count'] + 
                      normalized['video_share_count'] + 
                      normalized['video_comment_count']) / 4.0
        ...
        logger.info(f"{split}: popularity range=[{popularity.min():.4f}, {popularity.max():.4f}], mean={popularity.mean():.4f}")
    
    return result, min_max


class InformsDataset(Dataset):
    def __init__(self, data_path: str, popularity_labels: np.ndarray, max_frames: int = 100):
...
def compute_metrics(predictions, labels):
    ...
    mask = np.abs(labels) > 1e-10
    if mask.sum() > 0:
        mape = np.mean(np.abs((labels[mask] - predictions[mask]) / (labels[mask] + 1e-8))) * 100
    else:
        mape = 0.0
    ...
    try:
        if len(labels) < 2 or np.std(labels) < 1e-10 or np.std(predictions) < 1e-10:
            src = 0.0
        else:
            result = spearmanr(predictions, labels)
            src = float(result.correlation) if hasattr(result, 'correlation') else float(result[0])
...
class GSARJambaTOPOV2Model(nn.Module):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            total_loss = mse_loss + self.dppo_lambda * dppo_loss + load_balance_loss
...
def main():
    parser = argparse.ArgumentParser(description='GSAR-Jamba-TOPO Informs Training with Multimodal Memory Bank')
    parser.add_argument('--dataset_path', type=str, default='/root/GSAR-Jamba-TOPO-Public/data_v2/processed_features')
    ...
    parser.add_argument('--base_window_size', type=int, default=20)
    ...
    parser.add_argument('--dppo_lambda', type=float, default=0.05)
    parser.add_argument('--dppo_beta', type=float, default=0.3)
    ...
    # 处理自定义日志目录
    if args.log_dir:
        custom_log_dir = Path(args.log_dir)
        custom_log_dir.mkdir(parents=True, exist_ok=True)
        custom_log_file = custom_log_dir / f"train_informs_multimodal_{timestamp}.log"
        file_handler = logging.FileHandler(custom_log_file, encoding='utf-8')
        file_handler.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
        logger.addHandler(file_handler)
        logger.info(f"日志文件: {custom_log_file}")
    else:
    ...
    logger.info(f"Target: Composite Popularity (Normalized Average of 4 Metrics)")
    logger.info(f"日志文件: {log_file}")
    
    logger.info("=" * 60)
    logger.info("加载数据并计算复合热度...")
    logger.info("=" * 60)
    
    train_data = pickle.load(open(f"{args.dataset_path}/train.pkl", 'rb'))
    valid_data = pickle.load(open(f"{args.dataset_path}/valid.pkl", 'rb'))
    test_data = pickle.load(open(f"{args.dataset_path}/test.pkl", 'rb'))
    
    data = {'train': train_data, 'valid': valid_data, 'test': test_data}
    popularity_data, min_max = compute_composite_popularity(data)
    
    train_dataset = InformsDataset(f"{args.dataset_path}/train.pkl", popularity_data['train']['popularity'], args.max_frames)
    valid_dataset = InformsDataset(f"{args.dataset_path}/valid.pkl", popularity_data['valid']['popularity'], args.max_frames)
    test_dataset = InformsDataset(f"{args.dataset_path}/test.pkl", popularity_data['test']['popularity'], args.max_frames)
    ...
    # 使用多模态记忆库
    fixed_pmb_path = "/root/GSAR-Jamba-TOPO-Public/.fixed_assets/master_grid_M6_N32_informs_multimodal.pkl"
    ...
    logger.info("=" * 60)
    logger.info("GSAR-Jamba-TOPO Informs Training Configuration (Multimodal Memory Bank)")
    logger.info("=" * 60)
    logger.info(f"Random Seed: {args.seed}")
    logger.info(f"Device: {args.device}")
    logger.info(f"Model: GSAR-Jamba-TOPO-Composite with Multimodal Memory Bank")
    logger.info(f"Target: Composite Popularity (Normalized Average)")
    logger.info(f"Dataset: TikTok (Informs)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        logger.info(f"[ Epoch {epoch} (train) ]: total_loss = {train_metrics['total_loss']:.6f}, mse = {train_metrics['mse_loss']:.6f}, dppo = {train_metrics['dppo_loss']:.6f}, lb = {train_metrics['load_balance_loss']:.6f}")
        
        valid_metrics = evaluate(model, valid_loader, device)
        logger.info("=" * 60)
        logger.info("Valid Metrics (Composite Popularity [0, 1]):")
        ...
        logger.info("Test Metrics (Composite Popularity [0, 1]):")
        ...
        if valid_metrics['mape'] < best_valid_mape:
            old_best_mape = best_valid_mape
            old_best_mae = best_valid_mae
            old_best_mse = best_valid_mse
            
            if old_best_mape == float('inf'):
                logger.critical(f"=== Epoch {epoch}: Initializing best metrics ===")
                best_valid_mape = valid_metrics['mape']
                ...
                # 保存初始best模型
                if args.output_dir:
                    save_path = Path(args.output_dir) / "best_model_informs.pt"
                    torch.save({
                        'epoch': epoch,
                ...
                improvement_mae = (old_best_mae - valid_metrics['mae']) / old_best_mae * 100
                improvement_mse = (old_best_mse - valid_metrics['mse']) / old_best_mse * 100
                improvement_mape = (old_best_mape - valid_metrics['mape']) / old_best_mape * 100
                total_impr = improvement_mae + improvement_mse + improvement_mape
                
                logger.critical(f"Improvement: MAE=+{improvement_mae:.2f}%, MSE=+{improvement_mse:.2f}%, MAPE=+{improvement_mape:.2f}% (Total=+{total_impr:.2f}%)")
            ...
            logger.critical(f"No improvement. Early stop count: {early_stop_count + 1}/{args.early_stop}")
            early_stop_count += 1
    ...
    logger.info("=" * 60)
    logger.info(f"Target: Composite Popularity (Normalized Average)")
    logger.info("Best Test Results (Composite Popularity [0, 1]):")
    logger.info(f"  MAE:  {best_test_metrics['mae']:.6f}")
    logger.info(f"  MSE:  {best_test_metrics['mse']:.6f}")
    logger.info(f"  RMSE: {best_test_metrics['rmse']:.6f}")
