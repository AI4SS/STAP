"""
GSAR-Jamba-TOPO V2 端到端训练脚本 - 多模态记忆库版本
...
log_file = log_dir / f"train_topo_v2_multimodal_{timestamp}.log"
...
logger = logging.getLogger('GSAR_Jamba_TOPO_V2_Multimodal')
...
class SMPDataset(Dataset):
    def __init__(self, data_path: str, max_frames: int = 100):
...
def main():
    parser = argparse.ArgumentParser(description='GSAR-Jamba-TOPO V2 Training with Multimodal Memory Bank')
    parser.add_argument('--dataset_path', type=str, default='/root/GSAR-Jamba-TOPO-Public/data/processed_features')
    ...
    if args.log_dir:
        ...
        custom_log_file = custom_log_dir / f"train_topo_v2_multimodal_{timestamp}.log"
    ...
    train_dataset = SMPDataset(f"{args.dataset_path}/train.pkl", args.max_frames)
    valid_dataset = SMPDataset(f"{args.dataset_path}/valid.pkl", args.max_frames)
    test_dataset = SMPDataset(f"{args.dataset_path}/test.pkl", args.max_frames)
    ...
    # 使用多模态记忆库
    fixed_pmb_path = "/root/GSAR-Jamba-TOPO-Public/.fixed_assets/master_grid_M6_N32_multimodal.pkl"
    ...
    logger.info("=" * 60)
    logger.info("GSAR-Jamba-TOPO V2 Training Configuration (Multimodal Memory Bank)")
    logger.info("=" * 60)
    logger.info(f"Random Seed: {args.seed}")
    logger.info(f"Device: {args.device}")
    logger.info(f"Model: GSAR-Jamba-TOPO V2 with Multimodal Memory Bank")
    logger.info(f"Dataset: SMP-MRA")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if valid_metrics['mape'] < best_valid_mape:
            ...
            if old_best_mape == float('inf'):
                ...
                # 保存初始best模型
                if args.output_dir:
                    save_path = Path(args.output_dir) / "best_model_smp_mra.pt"
                    torch.save({
                        'epoch': epoch,
